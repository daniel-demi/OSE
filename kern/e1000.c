...
#define RX_QUEUE_NPAGES 64
#define BUFFS_IN_PAGE (PGSIZE / BUFF_SIZE)


// #define TX_QUEUE_AT(idx) (tx_queue_pages[idx / BUFFS_IN_PAGE] + (idx % BUFFS_IN_PAGE) * BUFF_SIZE)
#define RX_QUEUE_AT(idx) (rx_queue_pages[idx / BUFFS_IN_PAGE] + (idx % BUFFS_IN_PAGE) * BUFF_SIZE)

// LAB 6: Your driver code here
static volatile uint32_t *bar0;
struct e1000_tx_desc *tx_queue_desc;
// void *tx_queue_pages[TX_QUEUE_NPAGES];


struct e1000_rx_desc *rx_queue_desc;
void *rx_queue_pages[RX_QUEUE_NPAGES];
...
int attach_e1000(struct pci_func *pcif) {
	...
	int i, j;
	// for(i = 0 ; i < TX_QUEUE_NPAGES; i++) {
	// 	struct PageInfo *tx_page = page_alloc(ALLOC_ZERO);
	// 	tx_queue_pages[i] = page2kva(tx_page);
	// 	for (j = 0; j < BUFFS_IN_PAGE; j++) {
	// 		// tx_queue_desc[BUFFS_IN_PAGE * i + j].buffer_addr = (uint64_t)PADDR(TX_QUEUE_AT(i * BUFFS_IN_PAGE + j));
	// 		tx_queue_desc[BUFFS_IN_PAGE * i + j].buffer_addr = (uint64_t)PADDR(tx_queue_pages[i] + j * BUFF_SIZE);
	// 		tx_queue_desc[BUFFS_IN_PAGE * i + j].cmd = (1 <<3) | 1;
	// 		tx_queue_desc[BUFFS_IN_PAGE * i + j].status |= E1000_TXD_STAT_DD;
	// 	}
	// }
	for (j = 0; j < TRANS_QUEUE_SZ; j++) {
		// tx_queue_desc[BUFFS_IN_PAGE * i + j].buffer_addr = (uint64_t)PADDR(TX_QUEUE_AT(i * BUFFS_IN_PAGE + j));
		tx_queue_desc[j].buffer_addr = 0;
		tx_queue_desc[j].cmd = (1 <<3) | 1;
		tx_queue_desc[j].status |= E1000_TXD_STAT_DD;
	}
	...
	for(i = 0 ; i < RX_QUEUE_NPAGES; i++) {
		struct PageInfo *rx_page = page_alloc(ALLOC_ZERO);
		rx_queue_pages[i] = page2kva(rx_page);
		for (j = 0; j < BUFFS_IN_PAGE; j++) {
			// rx_queue_desc[BUFFS_IN_PAGE * i + j].buffer_addr = (uint64_t)PADDR(RX_QUEUE_AT(i * BUFFS_IN_PAGE + j));
			rx_queue_desc[BUFFS_IN_PAGE * i + j].buffer_addr = (uint64_t)PADDR(rx_queue_pages[i] + j * BUFF_SIZE);
